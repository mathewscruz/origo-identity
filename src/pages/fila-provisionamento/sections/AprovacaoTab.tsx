...
import { Check, X, Search, RefreshCw, AlertTriangle, ChevronLeft, ChevronRight, Sparkles, Bot } from "lucide-react";
...
import { QUEUE_ACTION_LABELS } from "@/lib/queueLabels";
...
const actionLabels: Record<string, string> = { ...QUEUE_ACTION_LABELS, enable: "Habilitar (AD)", sync_status_from_ad: "Sincronizar status ← AD" };
...
/**
 * Aba "Aguardando aprovação" da Fila de Provisionamento: decide (aprova/recusa) os itens
 * antes de irem ao Órigo Agente. Histórico/execução ficam na aba de itens da fila.
 */
export default function AprovacaoTab() {
  ...
  const [actionFilter, setActionFilter] = useState(searchParams.get("action") || "todos");
  ...
  useEffect(() => { setPage(0); setSelected(new Set()); }, [actionFilter, originFilter]);
  ...
  // ─── Paginated queue ───
  const queryKey = ["iam-approval-queue", page, actionFilter, originFilter, buscaDebounced];
  const { data: pageData, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      q = q.eq("status", "waiting_approval");
  ...
  // Realtime: useRealtimeSync (AppLayout) invalida as chaves "iam-*" quando iam_queue muda.

  // ─── Mutations ───
  const approveMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      // Decisão no banco: máquina de estados, approved_by = usuário logado, anti-auto-aprovação,
      // órfãos aprovados viram disable_entra, auditoria — tudo numa transação (RPC iam_queue_decidir).
      const { error } = await (supabase as any).rpc("iam_queue_decidir", { p_ids: ids, p_decisao: "approve", p_motivo: null });
      if (error) throw error;
    ...
    onSuccess: (ids) => {
      toast.success(`${ids.length} item(ns) aprovado(s) — o Órigo Agente executa na próxima leitura da fila`);
  ...
  const rejectMutation = useMutation({
    mutationFn: async ({ ids, reason }: { ids: string[]; reason: string }) => {
      ...
      const { error } = await (supabase as any).rpc("iam_queue_decidir", { p_ids: ids, p_decisao: "reject", p_motivo: reason || "Rejeitado" });
      if (error) throw error;
  ...
  // ─── Reconciliação (reconcile-identities): último job e progresso ───
  ...
  const { data: reconcileJob, refetch: refetchReconcileJob } = useQuery({
    queryKey: ["sync_jobs", "reconcile_identities"],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    if (reconcileJob.status === "done" && !sessionStorage.getItem(key)) {
  ...
  const reconcileMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await authedFetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
  ...
  const phaseLabels: Record<string, string> = {
    iniciando: "Iniciando…",
    carregando_colabs: "Carregando colaboradores…",
    baixando_entra: "Baixando Entra ID…",
    vinculando: "Vinculando colaboradores…",
    resolvendo_joiners: "Resolvendo joiners e criações abertas…",
    enfileirando_disable: "Conferindo desligados…",
    cobertura: "Analisando ativos…",
    orfaos: "Detectando contas órfãs…",
    done: "Concluído",
    erro: "Erro",
        ...
        <Card className="border-blue-300 bg-blue-50 dark:bg-blue-950/20">
          <CardContent className="py-4 flex items-center justify-between gap-4">
              ...
              <div className="text-sm">
                {reconcileRunning ? (
                  ...
                  <>
                    ...
                    <p className="text-blue-800 dark:text-blue-300/90">
                      A reconciliação compara a base com o Entra ID: vincula contas, cancela criações de quem já existe ou está desligado e detecta contas órfãs.
                    </p>
                  </>
                )}
              </div>
            ...
            >
              ...
              {reconcileRunning ? "Reconciliando…" : reconcileMutation.isPending ? "Iniciando…" : "Reconciliar identidades"}
            </Button>
          </CardContent>
        </Card>
      )}
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <CardDescription className="flex items-center gap-1">
                <Bot className="h-3.5 w-3.5" /> Quando ligado, nenhuma ação é entregue ao Órigo Agente sem aprovação de um administrador.
              </CardDescription>
            </div>
          ...
          <Card>
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <EmptyState message="Nada aguardando aprovação." />
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
      ...
      <AD open={reconcileOpen} onOpenChange={setReconcileOpen}>
        <ADContent>
          <ADHeader>
            ...
            <ADDesc>
              Vincula contas, cancela criações de quem já existe/está desligado, enfileira desabilitações de desligados e detecta órfãos. Roda em segundo plano.
            </ADDesc>
          </ADHeader>
          <ADFooter>
            <ADCancel>Cancelar</ADCancel>
